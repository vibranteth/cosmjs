...
describe("StargateClient", () => {
    ...
    describe("broadcastTx", () => {
        it("broadcasts a transaction", async () => {
            ...
            const authInfoBytes = (0, proto_signing_1.makeAuthInfoBytes)([{ pubkey, sequence }], feeAmount, gasLimit, feeGranter, feePayer);
        ...
        it("errors immediately for a CheckTx failure", async () => {
            ...
            const feeGranter = undefined;
            const feePayer = undefined;
            const authInfoBytes = (0, proto_signing_1.makeAuthInfoBytes)([{ pubkey, sequence }], feeAmount, gasLimit, feeGranter, feePayer, sequence);
            const chainId = await client.getChainId();
            const signDoc = (0, proto_signing_1.makeSignDoc)(txBodyBytes, authInfoBytes, chainId, accountNumber);
            const { signature } = await wallet.signDirect(address, signDoc);
            const txRaw = tx_1.TxRaw.fromPartial({
            ...
            try {
                ...
                (0, utils_1.assert)(false, "Expected broadcastTx to throw");
            }
            catch (error) {
                expect(error).toMatch((0, testutils_spec_1.simapp44Enabled)() ? /invalid recipient address/i : /Broadcasting transaction failed with code 7/i);
                (0, utils_1.assert)(error instanceof stargateclient_1.BroadcastTxError);
        ...
        it("respects user timeouts rather than RPC timeouts", async () => {
            ...
            const authInfoBytes1 = (0, proto_signing_1.makeAuthInfoBytes)([{ pubkey, sequence: sequence1 }], feeAmount, gasLimit, feeGranter, feePayer);
            ...
            const authInfoBytes2 = (0, proto_signing_1.makeAuthInfoBytes)([{ pubkey, sequence: sequence2 }], feeAmount, gasLimit, feeGranter, feePayer);
